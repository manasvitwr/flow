...
import { getTodayKey, FOCUS_KEY, TASKS_KEY, formatFocusTime } from './utils/timerUtils';
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem(FOCUS_KEY(), String(focusTime));
  }, [focusTime]);
  ...
  const handleTaskEnd = useCallback(() => {
    ...
    const completed = currentTask.duration;
    setFocusTime((prev: number) => prev + completed);

    if (currentTaskIndex < tasks.length - 1) {
      setCurrentTaskIndex((i: number) => i + 1);
    } else {
      setShowTaskManager(true);
    }
  }, [currentTaskIndex, tasks]);

  // Robust delta-time timer — resetKey=currentTaskIndex so only task switches reset the timer
  const {
    timeRemaining,
    isRunning,
    start: _start,
    ...
    subtractMinute: _subtractMinute,
    reset: _reset
  } = useTimer(tasks[currentTaskIndex]?.duration || 45 * 60, currentTaskIndex, handleTaskEnd);

  // Sync remaining time back into the task so the task list stays current
  const syncTaskDuration = useCallback((newTime: number) => {
    setTasks((prev: any[]) => {
      const updated = [...prev];
      if (updated[currentTaskIndex]) {
        updated[currentTaskIndex] = { ...updated[currentTaskIndex], duration: newTime };
      }
      return updated;
    });
  }, [currentTaskIndex]);

  const handleStart = _start;
  const handlePause = () => { _pause(); syncTaskDuration(timeRemaining); };
  const handleAddMinute = () => { _addMinute(); syncTaskDuration(Math.min(timeRemaining + 60, 59 * 60)); };
  const handleSubtractMinute = () => { _subtractMinute(); syncTaskDuration(Math.max(timeRemaining - 60, 0)); };
  const handleReset = () => { const dur = tasks[currentTaskIndex]?.duration || 45 * 60; _reset(dur); };

  // Play beep sound when timer changes
  useEffect(() => {
    if (isSoundEnabled && timeRemaining > 0 && timeRemaining % 60 === 0) {
      beepSound.current.play().catch(console.warn); // safety catch for autoplay block
    }
  ...
  const handleAddBookmark = (type: any, timeStr: any) => {
  ...
  const handleRemoveBookmark = (index: any) => {
  ...
  const handleReorderTasks = (fromIndex: any, toIndex: any) => {
  ...
  const handleEditTaskLabel = (newLabel: any) => {
    setTasks((tasks: any) => {
      const updated = [...tasks];
        ...
        <div className="timer-panel">
          <div style={{ width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', marginBottom: '0.5rem', gap: '0.5rem', position: 'relative' }}>
              {/* Focus Time Display */}
              <div style={{ fontWeight: 600, fontSize: '1.1rem', color: '#fff', marginRight: '1.5rem', fontFamily: isTamagotchiMode ? "'Minecraftia', monospace" : 'inherit', letterSpacing: '0.08em', textShadow: isTamagotchiMode ? '0 0 5px #fdfdfdff' : 'none', whiteSpace: 'nowrap' }}>
                Today's Focus: {formatFocusTime(focusTime)}
              </div>
