...
const styles = {
  appFooter: (isMobile: boolean, _isTamagotchi: boolean): React.CSSProperties => ({
    width: '100%',
    display: 'flex',
    flexDirection: (isMobile ? 'column' : 'row') as any,
    justifyContent: 'space-between',
    alignItems: isMobile ? 'center' : 'flex-end',
    padding: isMobile ? '2rem 4vw 60px 4vw' : '0 2vw 0px 2vw',
  ...
  title: (isMobile: boolean): React.CSSProperties => ({
    ...
    fontWeight: 400,
    fontSize: isMobile ? 'clamp(36px, 10vw, 64px)' : 'clamp(56px, 7vw, 90px)',
  ...
  tagline: (isMobile: boolean): React.CSSProperties => ({
    fontFamily: 'Inter, SF Pro, Helvetica Neue, Arial, sans-serif',
    fontWeight: 400,
    fontSize: isMobile ? 'clamp(28px, 6vw, 48px)' : 'clamp(40px, 5vw, 66px)',
    letterSpacing: '-0.04em',
    opacity: 0.5,
  ...
  iconsWrapper: (_isMobile: boolean): React.CSSProperties => ({
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    gap: '40px',
